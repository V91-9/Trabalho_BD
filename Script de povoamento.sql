...
VALUES ((SELECT ID_Loja FROM LOJA),
		(SELECT ID_Tipo_Produto FROM TIPO_PRODUTO WHERE Descricao = 'CJ'), 
		10, 'Jeans Preto');

INSERT INTO PEDIDO_ITENS (ID_Pedido, ID_Produto, ID_PI_Status_code)
VALUES ((SELECT ID_Pedido FROM PEDIDO), 
		(SELECT ID_Produto FROM PRODUTO), 
		(SELECT ID_PI_Status_code FROM PEDIDO_ITENS_STATUS_CODE WHERE Descricao = 'ON'));
		
INSERT INTO TRANSPORTE (ID_Pedido, ID_Transporte_Status_code)
VALUES ((SELECT ID_Pedido FROM PEDIDO), 
	(SELECT ID_Transporte_Status_code FROM TRANSPORTE_STATUS_CODE WHERE Descricao = 'NLJ'));
